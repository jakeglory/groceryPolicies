...
// Assign a policy set definition to a management group using data from an assignment JSON file
@description('Assignment object parsed from JSON')
param assignmentJson object
@description('Management group ID where the assignment will be created')
...
// Compose display name: [customer][scopeLast][domain] policyName
...
var _scopeDefault = managementGroupId
...
// Ensure assignment name meets 24-char limit
...
// Optional role assignments for the MI at the MG scope
@description('Optional array of roleDefinitionIds to grant to the assignment managed identity at the management group scope')
param roleDefinitionIds array = []

resource miRoleAssignments 'Microsoft.Authorization/roleAssignments@2022-04-01' = [for roleDefId in roleDefinitionIds: {
  name: guid(managementGroupId, assignment.name, roleDefId)
  properties: {
    roleDefinitionId: extensionResourceId(tenantResourceId('Microsoft.Management/managementGroups', managementGroupId), 'Microsoft.Authorization/roleDefinitions', roleDefId)
