...
param (
    ...
    [string]
    $definitionsPath = "lib/policy_definitions", #"lib/policy_definitions_ALDI", #
    [string]
    $definitionsLongPath = "$definitionsRoot/$definitionsPath",
    [string]
    $definitionsSetPath = "lib/policy_set_definitions", #""lib/policy_set_definitions_ALDI", 
    [string]
...
function New-PolicyDefinitionsBicepInputTxtFile {
    ...
    Get-ChildItem -Recurse -Path "$rootPath/$definitionsLongPath" -Filter "*.json" | ForEach-Object {
        ...
        Add-Content -Path "$rootPath/$definitionsLongPath/$defintionsTxtFileName" -Encoding "utf8" -Value "{`r`n`t`tname: '$policyDefinitionName'`r`n`t`tlibDefinition: json(loadTextContent('$definitionsPath/$fileName'))`r`n}"
    }
...
function New-PolicySetDefinitionsBicepInputTxtFile {
    ...
    Add-Content -Path "$rootPath/$definitionsSetLongPath/$defintionsSetTxtFileName" -Value "var customPolicySetDefinitionsArray = [" -Encoding "utf8" 
    ...
    Get-ChildItem -Recurse -Path "$rootPath/$definitionsSetLongPath" -Filter "*.json" -Exclude "*.parameters.json" | ForEach-Object {
        ...
        #$policySetDefParamVarCreation = $policySetDefParamVarCreation.Replace('-', "")
        $policySetDefParamVarCreation = ($policySetDefParamVarTrimJsonExt -replace '(?:_|-)(\p{L})', { $_.Groups[1].Value.ToUpper() }).TrimEnd('_')  #(?:^|_|-)(\p{L})
        $policySetDefParamVar = $policySetDefParamVarCreation + " = " + "loadJsonContent('$definitionsSetPath/$parametersFileName')"
            ...
            $policySetDefinitionsOutputForBicep.Keys | Sort-Object | ForEach-Object {
                ...
                if ($definitionReferenceIdForParameters.Contains("-") -or $definitionReferenceIdForParameters.Contains(" ") -or $definitionReferenceIdForParameters.Contains("\'")) {
                    ...
                    if ($definitionId.Contains('/')){
                        Add-Content -Path "$rootPath/$definitionsSetLongPath/$defintionsSetTxtFileName" -Encoding "utf8" -Value "`t`t`t`t{`r`n`t`t`t`t`t`tdefinitionReferenceId: '$definitionReferenceId'`r`n`t`t`t`t`t`tdefinitionId: '$definitionId'`r`n`t`t`t`t`t`tdefinitionParameters: $policySetDefParamVarCreation$definitionReferenceIdForParameters.parameters`r`n`t`t`t`t}"
                    }
                    else {
                        Add-Content -Path "$rootPath/$definitionsSetLongPath/$defintionsSetTxtFileName" -Encoding "utf8" -Value "`t`t`t`t{`r`n`t`t`t`t`t`tdefinitionReferenceId: '$definitionReferenceId'`r`n`t`t`t`t`t`tdefinitionId: $definitionId`r`n`t`t`t`t`t`tdefinitionParameters: $policySetDefParamVarCreation$definitionReferenceIdForParameters.parameters`r`n`t`t`t`t}"
                    }
    ...
    Add-Content -Path "$rootPath/$definitionsSetLongPath/$defintionsSetTxtFileName" -Encoding "utf8" -Value "`r`n]`r`n"
    # Add Policy Set/Initiative Definition Parameter Variables to Bicep Input File
...
function New-PolicyAssignmentsBicepInputTxtFile {
    ...
    Get-ChildItem -Recurse -Path "$rootPath/$assignmentsLongPath" -Filter "*.json" | ForEach-Object {
        ...
        Add-Content -Path "$rootPath/$assignmentsLongPath/$assignmentsTxtFileName" -Encoding "utf8" -Value "var policyAssignment$policyAssignmentNameNoHyphens = {`r`n`t`tdefinitionId: '$policyAssignmentDefinitionID'`r`n`t`tlibDefinition: json(loadTextContent('../../policy/$assignmentsLongPath/$fileName'))`r`n}`r`n"
    }
